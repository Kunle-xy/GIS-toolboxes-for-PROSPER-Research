...
class RuttingPolygonTool(object):
    def __init__(self):
        ...
        self.description = "Detects and visualizes rutting severity within a polygon using a DEM, with optional neighbor frequency study, also writes distress line features."
    ...
    def getParameterInfo(self):
        ...
        # NEW, output line features for detected distress
        param7 = arcpy.Parameter(
            displayName="Output Distress Line Features",
            name="out_lines",
            datatype="DEFeatureClass",
            parameterType="Required",
            direction="Output")

        params.extend([param0, param1, param2, param3, param4, param5, param6, param7])
        return params
    ...
    def updateParameters(self, parameters):
        # auto append .html for the plot output
    ...
    def updateMessages(self, parameters):
        ...
        # basic check for output line feature class location
        if parameters[7].valueAsText:
            ...
            if out_dir and not arcpy.Exists(out_dir):
                parameters[7].setErrorMessage("Output feature class location does not exist.")
        return

    def _create_out_lines_fc(self, out_fc, sref):
        """Create output polyline feature class with requested fields."""
        ...
        arcpy.management.CreateFeatureclass(out_path, out_name, "POLYLINE", spatial_reference=sref)

        # required fields
        arcpy.management.AddField(out_fc, "elevation_at_x", "DOUBLE")
        arcpy.management.AddField(out_fc, "distress_severity", "TEXT", field_length=16)

        # helpful fields
        arcpy.management.AddField(out_fc, "line_x", "DOUBLE")
        arcpy.management.AddField(out_fc, "depth_in", "DOUBLE")
        arcpy.management.AddField(out_fc, "area_ft2", "DOUBLE")
    ...
    def execute(self, parameters, messages):
        try:
            ...
            out_lines_fc = parameters[7].valueAsText
            ...
            with rasterio.open(dem_path) as src:
                ...
                # project polygon to DEM SRS if needed
                ...
                out_lines_fc = self._create_out_lines_fc(out_lines_fc, dem_srs)
                insert_fields = ["SHAPE@", "elevation_at_x", "distress_severity", "line_x", "depth_in", "area_ft2"]
                ...
                with arcpy.da.InsertCursor(out_lines_fc, insert_fields) as icur:
                    for f in frequencies:
                        ...
                        for line_idx, x in enumerate(x_steps):
                            ...
                            # sample DEM and keep coordinates aligned
                            depths = []
                            coords_xy = []
                            for pt in sample_points:
                                try:
                                    ...
                                    depth = np.nan
                                depths.append(depth)
                                coords_xy.append((pt.x, pt.y))

                            # drop NA consistently for depth and coordinates
                            if len(depths) < 2 or all(np.isnan(depths)):
                                continue
                            mask = [not np.isnan(d) for d in depths]
                            if sum(mask) < 2:
                                continue

                            depths_f = [d for d, m in zip(depths, mask) if m]
                            coords_f = [c for c, m in zip(coords_xy, mask) if m]
                            df_input = pd.DataFrame({"section": list(range(len(depths_f))), "depth": depths_f})
                                    ...
                                    if peak_indices:
                                        ...
                                        if valid_peaks:
                                            ...
                                            if (idx_h2 > idx_low > idx_h1 and
                                                ...
                                                # reset peaks to continue scanning
                                                peak_heights = [h2]
                                                peak_indices = [idx_h2]

                                direction_prev = direction

                            # finalize trailing pattern
                            ...
                            # stash elevation plots for single frequency view
                            if res:
                                ...
                                # push results, classify severity, write line features
                                for rut in res:
                                    ...
                                    if severity is None:
                                        continue  # ignore non classed ruts
                                    ...
                                    all_results[severity].append(rut_info)
                                    line_data.append(rut_info)
                                    line_data_all.append(rut_info)

                                    # build the horizontal line segment, start at the rut low point on the sampling line
                                    try:
                                        start_x, start_y = coords_f[idx_low]
                                    except Exception:
                                        # safe guard, skip if index mismatch
                                        continue

                                    end_x = start_x + dx
                                    end_y = start_y
                                    polyline = arcpy.Polyline(arcpy.Array([arcpy.Point(start_x, start_y),
                                                                            arcpy.Point(end_x, end_y)]),
                                                              dem_srs)

                                    elev_at_x = float(df_input["depth"].iloc[idx_low])  # DEM value at the low point, meters
                                    icur.insertRow([polyline,
                                                    elev_at_x,
                                                    severity,
                                                    float(x),
                                                    float(rut_info["depth"]),
                                                    float(rut_info["area"])])
            ...
            # plotting section unchanged except for earlier additions
            ...
            arcpy.AddMessage(f"Distress line features written to, {out_lines_fc}")
            arcpy.SetParameterAsText(7, out_lines_fc)
